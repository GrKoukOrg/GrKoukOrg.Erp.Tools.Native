...
public interface IBusinessServerDataAccess
{
     ...
     Task<BusinessApiResponse<IList<ItemListDto>>> GetBusinessServerItemsListV2Async();
...
public class BusinessServerHttpDataAccess : IBusinessServerDataAccess
{
    ...
    public async Task<BusinessApiResponse<IList<ItemListDto>>> GetBusinessServerItemsListV2Async()
    {
        _logger.LogInformation("GetBusinessServerItemsListV2Async");
        try
        {
            // Create HTTP client
            var client = _httpClientFactory.CreateClient("BusinessServerApi");
            ...
            // Deserialize JSON into a list of ItemListDto
            var items = JsonSerializer.Deserialize<BusinessApiResponse<IList<ItemListDto>>>(jsonContent);

            // Return the items (or an empty list if deserialization fails)
            return items ?? new BusinessApiResponse<IList<ItemListDto>>();
        }
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            // Timeout specific handling
